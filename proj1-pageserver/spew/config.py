...
def command_line_args():
    ...
    parser = argparse.ArgumentParser(
        description="Spew demonstrates configuring file access path")
    parser.add_argument("-D", "--debug", dest="DEBUG",
                        action="store_const", const=True,
                        help="Turn on debugging and verbose logging")
    parser.add_argument("-C", "--config", type=str, dest="CONFIG", 
                        help="Alternate configuration file")
    parser.add_argument("-R", "--docroot", type=str, dest="DOCROOT", 
                        help="Find file relative to this path")
    parser.add_argument("input",  
                            help="Text file; spew this to standard output")
...
def fake_cli_args():
    ...
    parser = argparse.ArgumentParser(description="Fake CLI for proxied service")
    cli_args = parser.parse_args([])
    log.debug("<- Command line args: {}".format(cli_args))
    return cli_args
