...
# 
# Using \n\n transmitted html without headers, updated to \r\n for 
# standard formatting
...
def respond(sock):
    ...
    parts = request.split()
    # Parts contain a GET requests with three parts
    # GET /classes HTTP/1.1 , indexing parts[1] gives you the site request
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        if (".." in source_path) or ("//" in source_path) or ("~" in source_path):
            ...
            if source_path.endswith(".html") or source_path.endswith(".css"):
                # HTML script would not run visually. Referenced from ChatGPT
                transmit("HTTP/1.0 200 OK\r\n", sock)
                transmit("Content-Type: text/html\r\n\r\n", sock)

              #  log.debug("Source path: {}".format(source_path))  # from spew.py
                try: 
